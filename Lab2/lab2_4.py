...
def Task4a():
    ...
    print("The probability of a Preposition (PP) being followed by a Verb (VB) is", tags["PP"].prob('VB'))
...
def Task4b():
    tagwords, tags = probDist(brown)
    prob = tags['START'].prob('PP') * tagwords['PP'].prob('I') * tags['PP'].prob('VB') * (
            tagwords['VB'].prob('conduct') * tags['VB'].prob('PP') * tagwords['PP'].prob('my') * (
            tags['PP'].prob('NN') * tagwords['NN'].prob('conduct') * tags['NN'].prob('END')
    ))
    print('The probability of the tags "PP VB PP NN" used for the following sequence of words:')
    print('"I conduct my conduct"?', prob)
...
#########################################################################################
def Task4c():
    print("You should invest in the stock market", end=" ")
    tags_words = get_tags(brown)
    distinct_tags = set([tag for (tag, word) in tags_words])
    tagwords, tags = probDist(brown)
    sentence = "You should invest in the stock market".split(' ')
    prob = ViterbiBestTag(distinct_tags, tagwords, tags, sentence)
    print(prob)
